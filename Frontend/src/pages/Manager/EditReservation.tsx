...
const EditReservation = () => {
  ...
  useEffect(() => {
    const fetchReservation = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/manager/reservations/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.put(`http://localhost:5000/manager/reservations/${id}`, payload);
