...
const ListeCars = () => {
  ...
  const fetchCars = async () => {
    const res = await axios.get("http://localhost:5000/manager/cars");
    setCars(res.data.cars);
  };

  const handleDeleteCar = async () => {
    if (!carToDelete) return;
    await axios.delete(`http://localhost:5000/manager/cars/${carToDelete._id}`);
  ...
  const handleAddCar = async (e) => {
    ...
    const res = await axios.post("http://localhost:5000/manager/cars", payload);
