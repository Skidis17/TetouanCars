...
const ListeClients = () => {
  ...
  useEffect(() => {
    const fetchClients = async () => {
      try {
        const response = await axios.get("http://localhost:5000/manager/clients");
  ...
  const handleAddOrEditClient = async () => {
    if (editMode) {
      // Edit client
      try {
        await axios.put(`http://localhost:5000/manager/clients/${newClient._id}`, newClient);
      ...
      // Add client
      try {
        const response = await axios.post("http://localhost:5000/manager/clients", newClient);
  ...
  const handleDeleteClient = async () => {
    ...
    try {
      await axios.delete(`http://localhost:5000/manager/clients/${clientToDelete._id}`);
