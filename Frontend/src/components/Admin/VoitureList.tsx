...
const VoitureList = () => {
  ...
  const fetchVoitures = async () => {
    try {
      const response = await axios.get("http://localhost:5000/api/voiture");
  ...
  const handleSubmit = async (formData: FormData) => {
    try {
      ...
      if (editingVoiture) {
        await axios.put(
          `http://localhost:5000/api/voiture/${editingVoiture._id}`,
          formData,
          config
        );
      } else {
        await axios.post(
          "http://localhost:5000/api/voiture",
          formData,
          config
        );
      }
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Voulez-vous vraiment supprimer cette voiture ?")) {
      try {
        await axios.delete(`http://localhost:5000/api/voiture/${id}`);
        fetchVoitures();
      } catch (error) {
  ...
  return (
    <div className="voiture-list-container">
      <div className="list-header">
        <h1 className="list-title">Gestion du Parc Auto</h1>
        <p className="list-subtitle">Liste complète des véhicules disponibles</p>
